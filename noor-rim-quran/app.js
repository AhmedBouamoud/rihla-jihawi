  ...
  $('encouragementTrackList').innerHTML = AudioManager.getTracks().map(t => `
    <div class="enc-track-row" data-id="${t.id}">
      <div class="enc-track-top">
        ...
        <input type="text" class="enc-name" data-id="${t.id}" value="${t.displayName}" maxlength="40" />
...
async function playAyahVoice(){
  const clip = await idbGet('ayahVoice', verseVoiceKey());
  if(clip){ stopAllAudio(); voicePlayer.src = URL.createObjectURL(clip); voicePlayer.play().catch(()=>{}); }
}
